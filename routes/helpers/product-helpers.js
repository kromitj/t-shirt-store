...
const filterByDeptnCat = (department, category, callback) => {
	if (!department) {
		...
		models.sequelize.query('call catalog_get_products_on_department(:inDepartmentId, :inShortProductDescriptionLength,:inProductsPerPage, :inStartItem)', { replacements: { inDepartmentId: parseInt(department), inShortProductDescriptionLength: 15, inProductsPerPage: 20, inStartItem: 0 } }).then((products) => {
		...
		models.sequelize.query('call catalog_get_category_products(:inCategoryId)', { replacements: { inCategoryId: category, inShortProductDescriptionLength: 15, inProductsPerPage: 20, inStartItem: 0 } }).then((products) => {
...
const filterByAtts = (attributes, products, callback) => {
	...
	product_ids.forEach((p_id, i) => {
		models.sequelize.query('call catalog_get_attributes_not_assigned_to_product(:inProductId)', { replacements: { inProductId: p_id } }).then((non_atts) => {
